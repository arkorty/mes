...
import { ISubSubCategory, ISubCategory, ICategory } from '../Models/Interface/ICategoryList';
                            ...
                            for (let index = 0; index < subCategories.length; index++) {
                                ...
                                if (currentSubCategory) {
                                    ...
                                    let subCategoryObj: ISubCategory = {
                                        ...
                                        imageUrl: (currentSubCategory.imageUrl) ? currentSubCategory.imageUrl : null,
                                        parentId: (currentSubCategory.parentId) ? currentSubCategory.parentId : null,
                                        subSubCategories: [],
                                        description: (currentSubCategory.description) ? currentSubCategory.description : ""
                                    }

                                    let subSubCategoryList: ISubSubCategory[] = [];
                                    let subSubCategories = await Category.find({ parentId: currentSubCategory._id });
                                    if (subSubCategories) {
                                        for (let index = 0; index < subSubCategories.length; index++) {
                                            ...
                                            if (subSubCategory) {
                                                ...
                                                const model: ISubSubCategory = {
                                                    ...
                                                    imageUrl: (subSubCategory.imageUrl) ? subSubCategory.imageUrl : null,
                                                    parentId: (subSubCategory.parentId) ? subSubCategory.parentId : null,
                                                    description: (subSubCategory.description) ? subSubCategory.description : ""
                                                }
            ...
            if (categories) {
                for (let index = 0; index < categories.length; index++) {
                    ...
                    if (category) {
                        ...
                        let subCategories = await Category.find({ parentId: new mongoose.Types.ObjectId(category._id) });
                            ...
                            for (let subIndex = 0; subIndex < subCategories.length; subIndex++) {
                                ...
                                if (subCategory) {
                                    const subModel: ICategory = {
                                        ...
                                        imageUrl: subCategory.imageUrl ? subCategory.imageUrl : null,
                                        parentId: subCategory.parentId ? subCategory.parentId : null,
